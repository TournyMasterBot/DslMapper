...
export default function RoomList() {
  ...
  const countAffected = (kind: Exclude<TargetKind,'room'|'unassigned'>, id: string) => {
  ...
  const handleAddRoomHere = () => {
    ...
    if (kind === 'unassigned' || kind === 'room') { closeMenu(); return }

    if (kind === 'world') {
      dispatch({ type: 'PATCH_ROOM', vnum, patch: { category: { worldId: id, continentId: undefined, areaId: undefined } as Category } })
    } else if (kind === 'continent') {
      const cont = catalog.continents[id]
      if (cont) dispatch({ type: 'PATCH_ROOM', vnum, patch: { category: { worldId: cont.worldId, continentId: id, areaId: undefined } as Category } })
    } else if (kind === 'area') {
      const area = catalog.areas[id]
      if (area) dispatch({ type: 'PATCH_ROOM', vnum, patch: { category: { worldId: area.worldId, continentId: area.continentId, areaId: id } as Category } })
    }
  ...
  const handleRename = () => {
    ...
    if (kind === 'room') {
      ...
      dispatch({ type: 'PATCH_ROOM', vnum: id, patch: { label: next.trim() || undefined } })
      closeMenu()
      return
    }
    ...
    if (kind === 'world') {
      const prev = catalog.worlds[id]; if (!prev) return
      dispatch({ type: 'CATALOG_UPSERT', payload: { kind: 'world', node: { ...prev, name: trimmed } } })
    } else if (kind === 'continent') {
      const prev = catalog.continents[id]; if (!prev) return
      dispatch({ type: 'CATALOG_UPSERT', payload: { kind: 'continent', node: { ...prev, name: trimmed } } })
    } else if (kind === 'area') {
      const prev = catalog.areas[id]; if (!prev) return
      dispatch({ type: 'CATALOG_UPSERT', payload: { kind: 'area', node: { ...prev, name: trimmed } } })
    }
  ...
  const onDropTarget = (e: React.DragEvent, kind: TargetKind, id: string) => {
    ...
    if (kind === 'unassigned') {
      dispatch({ type: 'PATCH_ROOM', vnum, patch: { category: { worldId: undefined, continentId: undefined, areaId: undefined } as Category } })
      return
    }
    if (kind === 'world') {
      dispatch({ type: 'PATCH_ROOM', vnum, patch: { category: { worldId: id, continentId: undefined, areaId: undefined } as Category } })
    ...
    if (kind === 'continent') {
      const cont = catalog.continents[id]
      if (!cont) return
      dispatch({ type: 'PATCH_ROOM', vnum, patch: { category: { worldId: cont.worldId, continentId: id, areaId: undefined } as Category } })
      return
    }
    if (kind === 'area') {
      const area = catalog.areas[id]
      if (!area) return
      dispatch({ type: 'PATCH_ROOM', vnum, patch: { category: { worldId: area.worldId, continentId: area.continentId, areaId: id } as Category } })
      return
    }
  }
  ...
  const contextItems = React.useMemo(() => {
    ...
    if (kind === 'world' || kind === 'continent' || kind === 'area' || kind === 'unassigned') {
      items.push({ label: 'Add Room here…', onClick: handleAddRoomHere })
      if (kind !== 'unassigned') items.push({ label: 'Rename…', onClick: handleRename })
      items.push({ label: 'Delete…', onClick: handleDelete, danger: kind !== 'unassigned' })
    } else if (kind === 'room') {
      ...
      <div>
        ...
        <div style={{ marginTop: 8 }}>
          ...
          >
            ...
            <span style={{ opacity:.7, fontSize:12 }}>({roomsByArea[UNASSIGNED]?.length ?? 0})</span>
          </div>
