...
// readable auto id if user leaves vnum blank
function genVnum() {
...
export default function RoomList() {
  ...
  const continentsByWorld: Record<
    string,
    Array<{ id: string; name?: string; worldId: string }>
  > = {};
  for (const c of Object.values(catalog.continents))
    (continentsByWorld[c.worldId] ||= []).push(c);
  for (const k in continentsByWorld) continentsByWorld[k].sort(sortByName);

  const areasByContinent: Record<
    string,
    Array<{ id: string; name?: string; worldId: string; continentId: string }>
  > = {};
  for (const a of Object.values(catalog.areas))
    (areasByContinent[a.continentId] ||= []).push(a);
  ...
  const toggle = (type: keyof TreeOpen, id?: string) => {
    setOpen((prev) => {
      if (type === "unassigned")
        return { ...prev, unassigned: !prev.unassigned };
      if (type === "worlds" && id)
        return { ...prev, worlds: { ...prev.worlds, [id]: !prev.worlds[id] } };
      if (type === "continents" && id)
        return {
          ...prev,
          continents: { ...prev.continents, [id]: !prev.continents[id] },
        };
      if (type === "areas" && id)
        return { ...prev, areas: { ...prev.areas, [id]: !prev.areas[id] } };
      return prev;
    });
  };
  ...
  const countAffected = (
    kind: Exclude<TargetKind, "room" | "unassigned">,
    id: string
  ) => {
    if (kind === "world")
      return Object.values(doc.rooms).filter((r) => r.category?.worldId === id)
        .length;
    if (kind === "continent")
      return Object.values(doc.rooms).filter(
        (r) => r.category?.continentId === id
      ).length;
    return Object.values(doc.rooms).filter((r) => r.category?.areaId === id)
      .length;
  };
  ...
  const handleDelete = () => {
    ...
    if (kind === "room") {
      const ok = confirm(`Delete room ${label}?`);
      if (!ok) return;
      dispatch({ type: "DELETE_ROOM", vnum: id });
      return;
    }
    if (kind === "unassigned") return;
    const n = countAffected(kind, id);
    const ok = confirm(
      `Delete ${kind} "${label}"?\nThis will unassign ${n} room(s) from this ${kind}${
        kind !== "area" ? " (and its descendants)" : ""
      }.`
    );
    if (!ok) return;
    dispatch({
      type: "CATALOG_DELETE",
      payload: { kind, id, mode: "unassign" },
    });
  };

  const handleAddRoomHere = () => {
    if (!menu.target) return;
    const { kind, id } = menu.target;

    let vnum = prompt("New room vnum? (leave blank to auto-generate)")?.trim();
    if (!vnum) vnum = genVnum();

    dispatch({ type: "ADD_ROOM", vnum });
    requestAnimationFrame(() => dispatch({ type: "SELECT_ROOM", vnum }));

    if (kind === "unassigned" || kind === "room") {
      closeMenu();
      return;
    }
  ...
  const handleRename = () => {
    ...
    if (kind === "room") {
      const next = prompt("Room label:", label) ?? "";
      if (next.trim() === label) return;
      dispatch({
        type: "PATCH_ROOM",
        vnum: id,
        patch: { label: next.trim() || undefined },
      });
      closeMenu();
      return;
    }
    ...
    const next = prompt(`Rename ${kind}:`, label) ?? "";
    const trimmed = next.trim();
    if (!trimmed || trimmed === label) return;

    if (kind === "world") {
      const prev = catalog.worlds[id];
      if (!prev) return;
      dispatch({
        type: "CATALOG_UPSERT",
        payload: { kind: "world", node: { ...prev, name: trimmed } },
      });
    } else if (kind === "continent") {
      const prev = catalog.continents[id];
      if (!prev) return;
      dispatch({
        type: "CATALOG_UPSERT",
        payload: { kind: "continent", node: { ...prev, name: trimmed } },
      });
    } else if (kind === "area") {
      const prev = catalog.areas[id];
      if (!prev) return;
      dispatch({
        type: "CATALOG_UPSERT",
        payload: { kind: "area", node: { ...prev, name: trimmed } },
      });
    }
  ...
  const contextItems = React.useMemo(() => {
    ...
    if (
      ...
      items.push({ label: "Add Room here…", onClick: handleAddRoomHere });
      if (kind !== "unassigned")
        items.push({ label: "Rename…", onClick: handleRename });
      items.push({
        label: "Delete…",
        onClick: handleDelete,
        danger: kind !== "unassigned",
      });
    } else if (kind === "room") {
      items.push({ label: "Rename…", onClick: handleRename });
      ...
      <div>
          ...
          return (
            <div key={w.id} style={{ marginBottom: 8 }}>
              <div
                ...
                onContextMenu={(e) =>
                  openMenu(e, "world", w.id, w.name || w.id)
                }
                    ...
                    return (
                      <div key={c.id} style={{ marginBottom: 6 }}>
                        <div
                          ...
                          onContextMenu={(e) =>
                            openMenu(e, "continent", c.id, c.name || c.id)
                          }
                          onDragOver={(e) =>
                            onDragOverTarget(e, "continent", c.id)
                          }
                          onDragLeave={(e) =>
                            onDragLeaveTarget(e, "continent", c.id)
                          }
                              ...
                              return (
                                <div key={a.id} style={{ marginBottom: 4 }}>
                                  <div
                                    ...
                                    onContextMenu={(e) =>
                                      openMenu(e, "area", a.id, a.name || a.id)
                                    }
                                    onDragOver={(e) =>
                                      onDragOverTarget(e, "area", a.id)
                                    }
                                    onDragLeave={(e) =>
                                      onDragLeaveTarget(e, "area", a.id)
                                    }
                                    onDrop={(e) =>
                                      onDropTarget(e, "area", a.id)
                                    }
                                    style={{
                                      cursor: "pointer",
                                      userSelect: "none",
        ...
        <div style={{ marginTop: 8 }}>
          <div
            ...
            onContextMenu={(e) =>
              openMenu(e, "unassigned", UNASSIGNED, "(Unassigned)")
            }
            ...
            style={{
              ...
              fontWeight: 600,
              padding: "2px 4px",
              ...dropClass("unassigned", UNASSIGNED),
            }}
          >
            {open.unassigned ? "▾" : "▸"} (Unassigned){" "}
            <span style={{ opacity: 0.7, fontSize: 12 }}>
              ({roomsByArea[UNASSIGNED]?.length ?? 0})
            </span>
          </div>

          {open.unassigned &&
            (roomsByArea[UNASSIGNED]?.length ? (
              <ul
                style={{
                  margin: "4px 0 0 18px",
                  padding: 0,
                ...
                {roomsByArea[UNASSIGNED].map((r) => (
                  <li key={r.vnum}>
                    <button
                      ...
                      onClick={() => selectRoom(r.vnum)}
                      onContextMenu={(e) =>
                        openMenu(e, "room", r.vnum, roomLabel(r.vnum, r.label))
                      }
                      style={{
                        background: "transparent",
                        border: "none",
